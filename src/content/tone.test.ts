...
/**
 * Страна / народ как субъект объяснения — корень, а не одна форма: «армян…» (армянин, армянский,
 * по-армянски), «Армени…» (Армения, Армении, из Армении), «Ереван». Тёплое держит CUSTOM, не NATION.
 */
const NATION = /армян|армени|ереван/i
...
/** Страна как устав: порядок или запрет объясняют отказ — «так принято», «нельзя», «лишают наследства», «отдают через год». */
const CUSTOM = /страна|обыча|принято|заведено|традици|закон|нельзя|лишают|обидел|отдают|наследств|через\s+год|никогда/i
/** Обобщение на народ: «твой армянин», «армяне не…»; народ — поручитель слова Алика («слово армянина», #280). */
const GENERALIZE = /твой армянин|армяне\s+(не|вечно|всегда)|слово\s+армянина/i
const bad = (t: string) => NATION.test(t) && (CAUSE.test(t) || CUSTOM.test(t) || GENERALIZE.test(t))
...
describe('тон: национальность — не оправдание', () => {
  ...
  it('негативный контроль: запретная связка ловится, тёплая — нет', () => {
    ...
    // формы, которые прежний сторож пропускал (#266): страна названа не словом «армян»
    expect(bad('Армения — страна, где деньги любят ждать.')).toBe(true)
    expect(bad('В Ереване пятница наступает после среды.')).toBe(true)
    expect(bad('Из Армении перевод идёт три недели.')).toBe(true)
    expect(bad('По-армянски долг отдают через год.')).toBe(true)
    // обе строки legends.ts до правки: обычай страны объясняет отказ
    expect(bad('К женщине в декрете за ключом? Брат, в Армении за такое лишают наследства. Даже того, которого нет.')).toBe(true)
    expect(bad('Свадьба идёт — выходить нельзя: в Армении кто вышел со свадьбы, тот обидел всю родню. Даже ради тебя.')).toBe(true)
    // откат строки клятвы (#280): Алик клянётся собой, а не народом
    expect(bad('Слово армянина: в пятницу. Слово армянина крепче банковского.')).toBe(true)
    // тёплое: народ назван (NATION срабатывает), но ничего не объясняет — false значим, а не пустой
    expect(NATION.test('На армянской свадьбе тосты до утра.')).toBe(true)
    expect(bad('На армянской свадьбе тосты до утра.')).toBe(false)
    ...
    expect(bad('Деньги на Евровидении, болеем за Армению.')).toBe(false)
  })
})
