...
const outcome = (id: string, when: R['when'], extra: Partial<R> = {}): R => ({
  ...
  respond: async ({ game }) => {
    ...
    if (o.sys) game.sys(o.sys.replace('{debt}', fmt(game.S.debt)))
...
export const paydayRules: R[] = [
  ...
  outcome('niva', [eq('finale.niva', 'chose')]),
  outcome('strasbourg', [is('ach.strasbourg')]),
  outcome('notyou', [exists('finale.razmik'), gte('count.rude', 8)]),
  outcome('default', []),
  {
