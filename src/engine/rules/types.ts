...
export interface Rule<G, E extends string = string, O = unknown> {
   ...
   * remember), и пробует следующее подходящее правило. Свои записи в игру (поле `S`, лента, тост, уведомление)
   * ответ обязан не делать, пока не решил говорить: их движок не видит (у игры это ловит строгий режим —
   * RuleSetOptions.silence). Триггеры промолчавшего правила не срабатывают: оно не случилось.
   */
